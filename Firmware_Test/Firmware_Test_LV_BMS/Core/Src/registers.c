...
void writeRegister(uint8_t address, uint8_t data){
    uint8_t buf[1], dataBuf[1];
    HAL_StatusTypeDef I2CStat;
    buf[0] = address; dataBuf[1] = data;

    I2CStat = HAL_I2C_Mem_Write(&hi2c1, BQ_I2CADDRESS, buf[0], 1, dataBuf, 1, HAL_MAX_DELAY);
    if (I2CStat!=HAL_OK){
        //Error?
        //Try again?
    }
...
uint8_t readRegister(uint8_t address){
    ...
    buf[0] = address;
    I2CStat = HAL_I2C_Mem_Read(&hi2c1,BQ_I2CADDRESS,buf[0],1,dataBuf,1,HAL_MAX_DELAY);
    if (I2CStat!=HAL_OK){
        //Error?
        //Try again?
    }
